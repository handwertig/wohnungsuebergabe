...
final class OwnersController {
    public function index(): void {
        Auth::requireAuth();
        $pdo = Database::pdo();
        $rows = $pdo->query('SELECT id,name,company,address,email,phone FROM owners ORDER BY name')->fetchAll(PDO::FETCH_ASSOC);
        ...
        foreach ($rows as $r) {
            $html .= '<tr><td>'.htmlspecialchars($r['name']).'</td><td>'.htmlspecialchars($r['company']??'').'</td><td>'.htmlspecialchars($r['email']??'').'</td><td>'.htmlspecialchars($r['phone']??'').'</td>'
                   . '<td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="/owners/edit?id='.$r['id'].'">Bearbeiten</a></td></tr>';
        }
    ...
    public function form(): void {
        ...
        if ($id) {
            $st = $pdo->prepare('SELECT * FROM owners WHERE id=?'); $st->execute([$id]); $row = $st->fetch(PDO::FETCH_ASSOC);
            if ($row) { $data = $row; }
        }
    ...
    public function save(): void {
        ...
        $pdo = Database::pdo();
        $id = $_POST['id'] ?? '';
        $fields = ['name','company','address','email','phone'];
        $vals = [$_POST['name'] ?? '', $_POST['company'] ?? null, $_POST['address'] ?? null, $_POST['email'] ?? null, $_POST['phone'] ?? null];
        if ($id) {
            $sql = 'UPDATE owners SET name=?, company=?, address=?, email=?, phone=? WHERE id=?';
            $stmt = $pdo->prepare($sql); $stmt->execute([$vals[0],$vals[1],$vals[2],$vals[3],$vals[4],$id]);
        } else {
            $sql = 'INSERT INTO owners (id,name,company,address,email,phone,created_at) VALUES (UUID(),?,?,?,?,?,NOW())';
