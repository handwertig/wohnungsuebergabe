...
final class ProtocolsController
{
    ...
    public function form(): void
    {
        ...
        // Datenschutzerklärung (aktuellste Version) LADEN — außerhalb von HTML!
        ...
        $cons = $meta['consents'] ?? [];
        $privacyChecked = !empty($cons['privacy']) ? ' checked' : '';
        ...
        // Tab Räume (mit Hinweisen IST-Zustand & Geruch)
        ...
        foreach ($rooms as $r) {
            ...
            $html .= '<div class="col-md-4"><label class="form-label">Geruch</label><input class="form-control" name="rooms['.self::h($rk).'][smell]" value="'.self::h($r['smell'] ?? '').'"><div class="form-text">Hinweis: Gerüche sind Teil des IST‑Zustands und sollten sachlich dokumentiert werden. Beschreiben Sie Art, Intensität und mögliche Ursache (z. B. „deutlicher Tabakgeruch im Wohnzimmer, auch nach Lüften wahrnehmbar“) – vermeiden Sie subjektive Bewertungen.</div></div>';
            ...
            if ($thumbs) {
                ...
                foreach ($thumbs as $t) {
                    if (!empty($t['url'])) $html .= '<a href="'.self::h($t['url']).'" target="_blank"><img src="'.self::h($t['url']).'" style="width:120px;height:80px;object-fit:cover;"></a>';
                }
                $html .= '</div></div>';
            } else {
        ...
        $html .= '<div class="col-md-4"><label class="form-label">Geruch</label><input class="form-control" name="rooms[__IDX__][smell]"><div class="form-text">Hinweis: Gerüche sind Teil des IST‑Zustands und sollten sachlich dokumentiert werden. Beschreiben Sie Art, Intensität und mögliche Ursache – vermeiden Sie subjektive Bewertungen.</div></div>';
        ...
        $html .= '<div class="col-12"><label class="form-label">IST‑Zustand</label><textarea class="form-control" rows="3" name="rooms[__IDX__][state]"></textarea><div class="form-text">Hinweis: Bitte beschreiben Sie den IST‑Zustand vollständig – positive wie negative Feststellungen; objektiv, präzise, konkret.</div></div>';
        ...
        $html .= '<div class="form-check mt-2"><input class="form-check-input" type="checkbox" name="meta[consents][privacy]"'.$privacyChecked.'> <label class="form-check-label">Ich habe die Datenschutzerklärung gelesen und akzeptiere sie.</label></div></div>';
    ...
    /** PDF-Export (letzte Version) */
    public function pdf(): void
    {
        $pdo = \App\Database::pdo();
        $id = (string)($_GET['protocol_id'] ?? '');
        if ($id === '') { http_response_code(400); echo "protocol_id fehlt"; return; }
    
        $st = $pdo->prepare("SELECT * FROM protocols WHERE id=? LIMIT 1");
        $st->execute([$id]);
        $p = $st->fetch(\PDO::FETCH_ASSOC);
        if (!$p) { http_response_code(404); echo "Protokoll nicht gefunden"; return; }
    
        // ⚠️ Minimaler PDF-Stub – später ersetzen wir das durch richtiges Template
    ...
    public function send(): void
    {
        ...
        // Mail vorbereiten (Stub)
        // TODO: SMTP aus Settings laden, PDF generieren, versenden, Versandlog schreiben
        echo json_encode([
            'status' => 'ok',
            'message' => "PDF an {$to} für Protokoll {$id} versendet (Stub)"
        ]);
    }
